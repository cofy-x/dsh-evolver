/** One isolated, offline, shipped Headless benchmark arm. No private host imports. */
...
const [harness, pairRoot, taskId, arm, behavior, resultFile] = process.argv.slice(2)
...
overlays.push(
  { id: 'agent-default-model', config: { provider: 'benchmark-script', model: 'offline-v1' } },
...
class Adapter extends LlmAdapter {
  ...
  async *stream(options) {
    ...
    if (requests >= BOUNDS.requestsPerRun || size > BOUNDS.inputPerRequest) {
      status = 'budget'
    ...
    async (host) => {
      ...
      fixture = await host.plugin({
        ...
        apply(scoped) {
          ...
          scoped.llm.registerAdapter(['benchmark-script'], new Adapter())
